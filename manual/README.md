...
JPA를 사용하게 됨으로서 의존성 문제가 발생하게 된다. 
...
(지금 코드에서는 다소 퉁쳐놓긴 했는데 사실 저러면 안된다.)
...
기존 Layered 구성에서는 이 코드를 작성하기가 상당히 난감했다. 
어디에 배치하는 것이 가장 좋을까?
api?? ==> 만들고 보니 api에 잔뜩 쌓여서 구별이 안 된다. 
위와 같은 상황들이 발생할 수 있다. 

특정 도메인에서 외부 api가 필요하다면 그냥 싹 다 output으로 내보내면 된다. 
그러니 위의 api 패키지에 싹 다 떄려 넣고 고통 받을 일이 없다는 뜻 
```
...
그렇기 떄문에 infrastructure 내부에 orm이 있고
실제 JPA와 R2DBC 요소들은 전부 이 orm 패키지에서 관리된다. 
그리고 순수 비즈니스 로직에 해당하는 domain은 세부 사항에 연결되지 않기 때문에 
순수하게 데이터 관점에서만 코드를 해석할 수 있다는 이점이 생긴다. 
고로 변경에 더 유연해진다.

비즈니스와 무관한 영역 (세부사항 - 기술 영역)
Redis, Mongo DB, MySQL, Kafka, RebbitMQ, Airflow 등등
어찌 보면 전푸 프레임워크라고 봐도 괜찮음
```
...
더 이상 infra(기술) 영역과 관계가 없는 순수 데이터가 되었다. 

도메인을 기준으로 데이터를 주고 받는다고 이해하면 된다. 
```