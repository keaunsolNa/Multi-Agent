...
async def create_account(request: CreateAccountRequest):
    account = await usecase.create_account(request.user_uuid, request.oauth_id, request.oauth_type, request.nickname, request.name, request.profile_image, request.email, request.phone_number, request.active_status, request.role_id)
